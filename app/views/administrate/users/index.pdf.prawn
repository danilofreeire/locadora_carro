...
Prawn::Document.generate("users_lista.pdf", page_size: "A4", margin: 0) do |pdf|
  ...
  pdf.bounding_box([20, pdf.bounds.top - 20], width: pdf.bounds.width - 40) do
    ...
    pdf.text "Lista de users", align: :center, size: 12, style: :bold, color: "000000"
    ...
    # Dados da tabela
    data = User.all.collect do |user|
      [user.id, user.nome, user.email, user.telefone, user.endereco]
    end
