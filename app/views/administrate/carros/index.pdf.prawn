...
Prawn::Document.generate("carros_lista.pdf", page_size: "A4", margin: 0) do |pdf|
  ...
  pdf.bounding_box([20, pdf.bounds.top - 20], width: pdf.bounds.width - 40) do
    ...
    pdf.text "Lista de Carros", align: :center, size: 12, style: :bold, color: "000000"

    # Espaçamento após o título
    pdf.move_down 30

    # Cabeçalho da tabela
    headers = ["ID", "Marca", "Modelo", "Ano", "Cor", "Placa", "Diária"]

    # Dados da tabela
    data = Carro.all.collect do |carro|
      [carro.id, carro.marca, carro.modelo, carro.ano, carro.cor, carro.placa, "R$ #{'%.2f' % carro.diaria}"]
    end
